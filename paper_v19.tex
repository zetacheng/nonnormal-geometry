...
\subsection{Noise injection pilot: hallucination frequency and confidence}
\label{sec:hallucination}

As a pilot study, we inject noise matrices with controlled operator
geometry into a toy classifier (5-class, 2-layer MLP,
$d_{\mathrm{hidden}} = 32$, 70.8\% clean accuracy). Three noise types
are tested: \textsc{Growth} (Henrici $= 0.987$, transient growth
$\mathcal{G} = 111.7$), \textsc{No-growth} (Henrici $= 0.162$,
$\mathcal{G} = 1.52$), and \textsc{Normal} (symmetric,
$\mathcal{G} = 1.0$). All matrices are normalized to equal operator
norm.

\begin{table}[ht]
\centering
\caption{Toy classifier: error characteristics under operator-norm-matched noise.}
\label{tab:hallucination}
\begin{tabular}{@{}lccc@{}}
\toprule
Noise type & Error rate & Conf.\ on errors & High-conf errors ($>0.8$) \\
\midrule
Growth       & 31.5\% & 0.507 & 2.1\% \\
No-growth    & 30.5\% & 0.481 & 1.1\% \\
Normal       & 31.7\% & 0.528 & 5.3\% \\
\bottomrule
\end{tabular}
\end{table}

Error rates are nearly identical (${\sim}31\%$), confirming that
error \emph{frequency} depends on noise magnitude, not geometry.
Confidence on errors, however, differs: growth noise produces the
most confident errors (0.507), consistent with the interpretation
that transient amplification converts low-confidence errors into
high-confidence ones.

\textbf{Caveat and preview.}
The toy model ($d = 32$, 2 layers) lacks the representational depth
for transient amplification to serve as an implicit detection signal.
On the GPU-scale Transformer ($d = 384$, 6 layers), we observe a
\emph{reversal}: growth noise \emph{reduces} confidence on errors
because the large transient perturbations trigger confidence
recalibration (Section~\ref{sec:noise-injection}). The GPU experiment,
which uses a strength-sweeping design rather than operator-norm
matching, is the primary result; the toy experiment correctly
identifies the noise--error relationship but not the confidence
recalibration mechanism that emerges at scale.

\subsection{Transition to GPU-scale experiments}

The toy experiments establish the core phenomena: PRC significantly
outperforms baselines ($p < 0.01$), destructive residuals emerge
at specific layers, and the adversarial balance between normal and
non-normal components produces intermediate curvature. However,
the toy task does not require transient amplification (Group~D is
competitive), and the small model dimension ($d = 24$) limits the
scope of non-normality. We now test whether these findings scale to
vision (CIFAR-10, Section~\ref{sec:cifar}) and language modeling
(WikiText-103, Section~\ref{sec:lm}), where higher task complexity
and intrinsic uncertainty should amplify the expressivity--stability
tradeoff.
...
\begin{table}[ht]
\centering
\caption{Statistical comparisons for CIFAR-10 (paired $t$-tests,
$n = 3$ seeds, Holm-corrected across 5 comparisons).}
\label{tab:cifar-stats}
\begin{tabular}{@{}lcccl@{}}
\toprule
Comparison & Diff & 95\% CI & $p$ & Holm \\
\midrule
C $-$ D & $+2.86\%$ & $[+1.93, +3.79]$ & 0.006 & reject$^\dagger$ \\
B $-$ D & $+3.14\%$ & $[+1.30, +4.98]$ & 0.018 & n.s.\ (secondary) \\
B $-$ A & $+0.20\%$ & $[-1.45, +1.84]$ & 0.66  & n.s. \\
C $-$ A & $-0.08\%$ & $[-0.93, +0.76]$ & 0.71  & n.s. \\
C $-$ B & $-0.28\%$ & $[-1.24, +0.68]$ & 0.34  & n.s. \\
\bottomrule
\end{tabular}
\end{table}
...
The C~vs.~D comparison ($+2.86\%$, $p = 0.006$, 95\% CI $[+1.93, +3.79]$,
Cohen's $d = 7.66$) survives Holm correction; B~vs.~D ($+3.14\%$,
$p = 0.018$, Cohen's $d = 4.25$) narrowly misses the Holm threshold
and is reported as a secondary finding.
Eliminating non-normality by symmetrizing weights causes a large,
statistically significant loss of expressivity on a real vision task.
Unlike the toy experiments where Group~D was competitive, CIFAR-10's
richer feature structure requires the transient amplification that
...
H $-$ A & $-0.0456$ & $[-0.048, -0.043]$ & 0.0001 & reject \\
C $-$ A & $+0.0130$ & $[+0.011, +0.015]$ & 0.0012 & reject \\
D $-$ A & $-0.0193$ & $[-0.023, -0.016]$ & 0.0021 & reject \\
G $-$ A & $+0.0001$ & $[-0.001, +0.001]$ & 0.79   & n.s. \\
\midrule
\multicolumn{5}{@{}l}{\emph{High-confidence errors}} \\
H $-$ A & $-0.0064$ & $[-0.007, -0.006]$ & 0.0010 & reject \\
\bottomrule
\end{tabular}
\end{table}
...
curvature of Group~A's FFN weights grows substantially: Layer~5's
curvature increases from 10.9 to 50.0 (4.6$\times$), and Layer~4's
excess noise grows from 146 to 401 (2.7$\times$). The correlation
between curvature and excess noise across layers is $r = 0.77$.
These trends are consistent across all 3 seeds: cross-seed
curvature differs by ${\leq}8\%$ per layer
(Layer~0: 3.2\%, Layer~1: 8.0\%, Layer~2: 7.3\%,
Layer~3: 6.1\%, Layer~4: 3.0\%, Layer~5: 4.8\%;
means: 44.5, 26.3, 28.1, 37.9, 47.5, 48.5).
...
\textbf{Scale dependence.}
The CPU pilot (Section~\ref{sec:hallucination}) found the
\emph{opposite} confidence ordering: growth noise produced
\emph{higher} ConfOnErr at toy scale ($d = 32$, 2 layers).
The reversal reflects model capacity: the shallow MLP cannot
detect and respond to transient spikes, so all non-normal noise
increases confidence indiscriminately. The 6-layer Transformer
has sufficient depth for confidence recalibration to emerge.
This scale dependence is itself informative: it predicts that
larger models may develop even stronger recalibration of
growth-type perturbations while remaining vulnerable to
persistent, non-amplifying noise.
...
support a more nuanced geometric account of confident model errors
...
All training code, analysis scripts, and model checkpoints for
reproducing the experiments in this paper are available at
\url{https://github.com/zetacheng/nonnormal-geometry}\footnote{Checkpoints
available on request due to file size (${\sim}120$\,MB each).}. The
repository includes: (1)~Phase~1--2 training scripts for Groups A--D,
G, and H on both CIFAR-10 and WikiText-103; (2)~Phase~3 analysis
code producing all tables, figures, and statistical tests;
(3)~Phase~4 noise injection code with pre-constructed noise matrices;
(4)~pre-trained checkpoints for all groups and seeds. WikiText-103
is publicly available via HuggingFace; CIFAR-10 is available via
\texttt{torchvision}.
